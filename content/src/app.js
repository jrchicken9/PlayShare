...
export function runPlayShareContent() {
  ...
  /** Floating marker dashboard hidden by user (×); reset when a new recording starts. */
  ...
  async function uploadAnonymizedDiagnosticExport() {
    ...
    try {
      const deepPref = await new Promise((r) =>
        chrome.storage.local.get(['playshare_diag_upload_deep'], r)
      );
              ...
              if (res && res.ok && res.reportId) {
                ...
                const u404 =
                  st === 404
                    ...
                    : '';
                const u503 =
                  st === 503 && String(errCode).includes('hash_salt')
                    ? ' Set env PLAYSHARE_DIAG_HASH_SALT on the server (16+ random characters) in Railway, then redeploy.'
                    : st === 503
                      ? ' Server misconfigured or unavailable (check Railway logs).'
                      : '';
                const u401 =
                  st === 401 && /unauthorized/i.test(String(errCode))
                    ? ' Refresh the upload access secret in Analytics so the extension can mint a fresh scoped upload token, or remove the upload secret env vars on the server.'
                    : '';
                const u500store =
  ...
  function ensureRecorderMarkerDashboard() {
    ...
    dash.innerHTML = `
      ...
      <div class="ws-diag-marker-dash-grid">
        ${DIAG_RECORDER_MARKER_PRESETS.map(
          (p) =>
            `<button type="button" class="ws-diag-marker-dash-chip" data-recorder-marker="${p.code}" title="${escapeAttrForTitle(
              p.hint || p.label
            )}">${p.label}</button>`
        ).join('')}
      </div>
    ...
    if (customBtn) {
      customBtn.addEventListener('click', () => {
        if (!getVideoProfiler().isRecording()) return;
        const raw =
          typeof window !== 'undefined' && window.prompt ? window.prompt('Short note (optional, no preset code):', '') : '';
    ...
    recorderMarkerDashEl = /** @type {HTMLDivElement} */ (dash);
