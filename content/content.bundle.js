...
`;
    ...
    async function copyDiagExport() {
      ...
      if (navigator.clipboard?.writeText) {
        navigator.clipboard.writeText(json).then(() => diagLog("DIAG_EXPORT", { copied: true, unified: true })).catch(() => diagLog("ERROR", { message: "Copy failed" }));
      } else {
    ...
    async function downloadDiagExport() {
      ...
      const host = (payload.extension && payload.extension.pageHost ? String(payload.extension.pageHost) : typeof location !== "undefined" ? location.hostname : "page").replace(/[^a-z0-9.-]/gi, "_") || "page";
    ...
    async function downloadUnifiedExportWithProfilerFrame() {
      ...
      const a = document.createElement("a");
      a.href = URL.createObjectURL(blob);
      const host = (payload.extension && payload.extension.pageHost ? String(payload.extension.pageHost) : "page").replace(/[^a-z0-9.-]/gi, "_") || "page";
...
--- Unified JSON ---
Use **Export report** or **Copy JSON** for one file: extension report (${extension.reportSchemaVersion || "?"}), video profiler (${st.snapshotCount} snapshots), ${siteSync.key === "prime" ? "Prime player/site digest (captured at export)." : "no Prime site block."} Does not include the separate Prime missed-ad-only JSON.
`;
      const narrative = (extension.narrativeSummary || "") + appendix;
      if (navigator.clipboard?.writeText) {
        navigator.clipboard.writeText(narrative).then(() => diagLog("DIAG_EXPORT", { narrativeCopied: true, unified: true })).catch(() => diagLog("ERROR", { message: "Copy failed" }));
      } else {
        diagLog("ERROR", { message: "Clipboard API unavailable" });
      }
    }
      ...
      }
      {
        ...
        if (videoProfilerStatusEl) {
          try {
            ...
            const durSec = st.startedAtMs != null && endMs != null ? Math.max(0, Math.round((endMs - st.startedAtMs) / 1e3)) : null;
            ...
            const topCounts = Object.entries(st.eventTypeCounts).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([k, n]) => `${k}×${n}`).join(" · ");
            const um = st.userMarkerCount != null ? st.userMarkerCount : 0;
            const lim = st.recordingLimits;
            const capLine = lim && typeof lim.approxMaxWallMinutes === "number" ? `Every ${lim.snapshotIntervalMs / 1e3}s · keeps last <strong>${lim.maxSnapshots}</strong> snaps (~<strong>${lim.approxMaxWallMinutes}</strong> min wall if full) · max <strong>${lim.maxEvents}</strong> events (ring buffer).` : "";
        ...
        try {
          ...
          if (profilerStartBtn) {
            ...
            profilerStartBtn.title = recording ? "Already recording — use Stop when finished" : !canStart ? "Start playback first — recording needs a video element" : "Start capturing profiler snapshots into the unified export";
          }
          if (profilerStopBtn) {
            profilerStopBtn.disabled = !recording;
            profilerStopBtn.title = recording ? "Stop capturing (freeze profiler timeline for export)" : "Not recording — nothing to stop";
          }
          ...
          if (recToggle && recLabel) {
            ...
            recToggle.title = recording ? "Stop profiler recording" : !canStart ? "Start playback first — then record" : "Start profiler recording (included in unified export)";
          }
        } catch {
    ...
    `;
      ...
      <div class="ws-diag-compact-root ws-diag-body-scroll">
        ...
        <div class="ws-diag-compact-snaps">
          ...
          ${siteSync.key === "prime" ? `<button type="button" class="ws-diag-btn ws-diag-btn-sm ws-diag-btn-missed-ad diag-prime-missed-ad-btn" title="Missed-ad investigation only (not in unified export)">Ad snap</button>
            <button type="button" class="ws-diag-btn ws-diag-btn-sm ws-diag-btn-primary-compact" id="diagCompactUnifiedExport" title="Download full report: extension + recording + Prime">Export</button>` : '<button type="button" class="ws-diag-btn ws-diag-btn-sm ws-diag-btn-primary-compact" id="diagCompactExport" title="Download full report: extension + recording">Export</button>'}
        </div>
      </div>
